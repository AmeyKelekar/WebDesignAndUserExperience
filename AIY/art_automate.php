				...
				<div class="content-right twelve columns">
					<div id="content">
						<?php
							...
							$id = $_GET['id'];
							$typ=$_GET['type']."_db";
							$query=mysql_query("SELECT * FROM $typ WHERE mbid=0 AND pid='$person_id' AND artid=$id");
							for($l=0;$l<5;$l++)
							{
								$sum[$l]=0;
							}
							$mid=mysql_result($query,0,"artid");
							$query1=mysql_query("SELECT * FROM art_db WHERE mbid='$mid'");
							$nn=mysql_num_rows($query1);
							for($j=0;$j<$nn;$j++)
							{
								if(mysql_result($query1,$j,"type")==mysql_result($query,0,"type"))
									$a_type+=100;
								if(mysql_result($query1,$j,"painter")==mysql_result($query,0,"painter"))
								{
									$a_painter+=200;	
								}
								else
								...
								if(mysql_result($query1,$j,"year")==mysql_result($query,0,"year"))
								{
									$a_year+=100;	
								}
								else
								...
								$sum[$j]+=$a_type+$a_painter+$a_year;
								if(mysql_result($query1,$j,"abase")<=mysql_result($query,0,"abase"))
								{
									$sum[$j]+=100;
								}
								else
							...
							if($nn!=0)
							{
								...
								echo "<br/>";
								echo "<font color='red'>";
								echo mysql_result($query1,$max_id,"type")." ";
								echo mysql_result($query1,$max_id,"painter")." ";
								echo mysql_result($query1,$max_id,"year")." ";
								
								echo mysql_result($query1, $max_id, "abase")." ";
								echo "</font>";
								echo "<br/>";
								echo "</font>";
							}
