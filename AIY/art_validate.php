				...
				<div class="content-right twelve columns">
					<div id="content">
						<?php
							...
							$year=$_POST['year'];
							$artbase=$_POST['artbase'];
							$comments=$_POST['comments'];
							$flag=0;
							if($arttype&&$painter&&$year&&$artbase&&$comments)
							{
								if(!ctype_alnum(str_replace(array(' ',"'",'-'),'',$arttype)) && (!is_numeric($arttype[0])))
								{
									$flag+=1;
									echo "<font face='Courier New' size='+1' color='red'>";
									echo "Type of Art is not accepted.<br/>";
									echo "</font>";
								}
								if(!ctype_alnum(str_replace(array(' ',"'",'-'),'',$painter)) && (!is_numeric($painter[0]))) 
								{
									$flag+=1;
									echo "<font face='Courier New' size='+1' color='red'>";
									echo "The painter name is not valid<br/>";
									echo "</font>";
								}
								...
								if(!is_numeric($artbase))
								{
									$flag+=1;
									echo "<font face='Courier New' size='+1' color='red'>";
									echo "The base price is not valid<br/>";
									echo "</font>";
								}
							...
							else
							{
								$flag+=1;
								echo "<font face='Courier New' size='+1' color='red'>";
								echo "Enter the required details<br/>";
								echo "</font>";
							}
						?>
						<?php
							...
							{
								echo "<font face='Courier New' size='+1' color='red'>";
								echo "Your bid has been successfully submitted";
								echo "</font>";
								mysql_query("INSERT INTO art_db (`type`, `painter`, `year`, `abase`,`mbid`,`pid`, `comments`) VALUES ('$arttype', '$painter', '$year', '$artbase','$mbid', '$pid','$comments');");
							}
							else
							{
								echo "<font face='Courier New' size='+1' color='red'>";
