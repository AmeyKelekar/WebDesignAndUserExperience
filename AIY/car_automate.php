				...
				<div class="content-right twelve columns">
					<div id="content">
						<?php
							...
							$id = $_GET['id'];
							$typ=$_GET['type']."_db";	
							$query=mysql_query("SELECT * FROM $typ WHERE mbid=0 AND pid='$person_id' AND carid='$id'");
							for($l=0;$l<5;$l++)
							{
								$sum[$l]=0;		
							}
							$mid=mysql_result($query,0,"carid");
							$query1=mysql_query("SELECT * FROM car_db WHERE mbid='$mid'");
							$nn=mysql_num_rows($query1);
							for($j=0;$j<$nn;$j++)
							{
								if(mysql_result($query1,$j,"brand")==mysql_result($query,0,"brand"))
									$c_brand+=100;
								...
								$sum[$j]+=$c_brand+$c_model+$c_color;
								if(mysql_result($query1,$j,"basep")<=mysql_result($query,0,"basep"))
								{
									$sum[$j]+=100;
								}
								else
							...
							if($nn!=0)
							{
								...
								echo "<br/>";
								echo "<font color='red'>";
								echo mysql_result($query1,$max_id,"brand")." ";
								echo mysql_result($query1,$max_id,"model")." ";
								echo mysql_result($query1,$max_id,"color")." ";
								echo mysql_result($query1, $max_id, "basep")." ";
								echo "</font>";
								echo "<br/>";
								echo "</font>";
							}
