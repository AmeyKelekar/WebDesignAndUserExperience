				...
				<div class="content-right twelve columns">
					<div id="content">
						<?php
							...
							$compbrand=$_POST['compbrand'];
							$os=$_POST['os'];
							$compcolor=$_POST['compcolor'];
							$ram=$_POST['ram'];
							$processor=$_POST['processor'];
							$hdd=$_POST['hdd'];
							$gc=$_POST['gc'];
							$compbase=$_POST['compbase'];
							$comments=$_POST['comments'];
							$flag=0;

							if($compbrand&&$os&&$compcolor&&$ram&&$processor&&$hdd&&$gc&&$compbase&&$comments)
							{ 
								if(!ctype_alnum(str_replace(array(' ',"'",'-'),'',$compbrand)) && (!is_numeric($compbrand[0])))
								{			
									$flag+=1;
									echo "<font face='Courier New' size='+1' color='red'>";
									echo "Type of Computer Brand is not accepted.<br/>";
									echo "</font>";
								}
								if(!ctype_alnum(str_replace(array(' ',"'",'-'),'',$os)) && (!is_numeric($os[0])))
								{
									$flag+=1; 
									echo "<font face='Courier New' size='+1' color='red'>";
									echo "Enter valid operating system<br/>";
									echo "</font>";
								}
								if(!ctype_alnum(str_replace(array(' ',"'",'-'),'',$compcolor)) && (!is_numeric($compcolor[0])))
								{
									$flag+=1;
									echo "<font face='Courier New' size='+1' color='red'>";
									echo "Enter valid computer color<br/>";
									echo "</font>";
								}
								if(!ctype_alnum(str_replace(array(' ',"'",'-'),'',$ram)))
								{		
									$flag+=1;
									echo "<font face='Courier New' size='+1' color='red'>";
									echo "Enter valid ram details<br/>";
									echo "</font>";
								}
								if(!ctype_alnum(str_replace(array(' ',"'",'-'),'',$processor)) && (!is_numeric($processor[0])))
								{
									$flag+=1;
									echo "<font face='Courier New' size='+1' color='red'>";
									echo "Enter valid processor details<br/>";
									echo "</font>";
								}
								if(!ctype_alnum(str_replace(array(' ',"'",'-'),'',$hdd)))
								{		
									$flag+=1;
									echo "<font face='Courier New' size='+1' color='red'>";
									echo "Enter valid Hard Disk details<br/>";
									echo "</font>";
								}
								if(!ctype_alnum(str_replace(array(' ',"'",'-'),'',$gc)))
								{
									$flag+=1;
									echo "<font face='Courier New' size='+1' color='red'>";
									echo "Enter valid Graphic card details<br/>";
									echo "</font>";
								}
								if(!is_numeric($compbase))
								{
									$flag+=1;
									echo "<font face='Courier New' size='+1' color='red'>";	
								...
								if ( strlen( $comments ) == 0 )
								{
									$flag+=1;
									echo "<font face='Courier New' size='+1' color='red'>";
									echo "Post some additional details<br/>";
									echo "</font>";
								}
							}
							else
						...
						<?php
							...
							mysql_select_db('phpregister') or die(mysql_error());
							/* for uid */
							...
							if($flag==0)
							{
								...
								echo "</font>";
								mysql_query("INSERT INTO comp_db (`cbrand`, `os`, `color`, `ram`, `processor`, `hdd`, `graphic`, `cbase`,`mbid`, `pid`,`comments`) VALUES ('$compbrand', '$os', '$compcolor', '$ram', '$processor', '$hdd', '$gc', '$compbase', '$mbid','$pid','$comments')");
							}
							else
							{	
								echo "<font face='Courier New' size='+1' color='red'>";
